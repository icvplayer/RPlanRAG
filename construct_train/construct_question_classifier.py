    ...
    for i, dataset_name in enumerate(dataset_names):
        ...
        for i, x in enumerate(tqdm(dataloader, desc="Generating")):
            ...
            if args.use_planning:
                output = rag.query_with_plan(query, q_param, planning_model, summary_model)
            else:
            ...
            if accelerator.process_index == 0:
                ...
                answers = raw_dataset_subset["answers"]  # 真实的答案标签
                all_classes = []
                # all_classes = raw_dataset_subset["all_classes"][0]
                score = scorer(dataset_name, preds, answers, all_classes) # 得到特定任务对应的分数
                acc_score_ = acc_score(preds, answers)  # 添加acc_score分数
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gen_llm_name", type=str, default="mistral:7b-instruct-16k", help="The model name which is used to generate triples")
    ...
    parser.add_argument("--use_summary", type=bool, default=False, help="Wether to use summary model")
