...
def get_qa_instruction(
    question: str, context: List, RETRIEVAL_TOKEN, use_cot):
    if not question:
    ...
    for document_index, document in enumerate(context):
        ...
        document_prompt = f"[{document_index+1}](Title: {document['title']}) {document_text}"  # document_index+1应该是代表的第几个引用
        formatted_documents.append(document_prompt)
    return prompt_template.format(question=question, search_results="\n".join(formatted_documents))


def get_instruction_dataset(dataset, max_prompt_length, tokenizer, use_cot, RETRIEVAL_TOKEN, sample_answer=True):
    instruction_dataset = []
...
def load_hotpotqa_dataset(input_path, max_prompt_length, tokenizer, use_cot=False, RETRIEVAL_TOKEN='<R>'):
    train_data, test_data = load_hotpotqa_data(input_path)
    instruction_dataset_train = get_instruction_dataset(train_data[:], max_prompt_length, tokenizer, use_cot, RETRIEVAL_TOKEN)  # 得到规范形式的数据集
    instruction_dataset_test = get_instruction_dataset(test_data[:], max_prompt_length, tokenizer, use_cot, RETRIEVAL_TOKEN)
