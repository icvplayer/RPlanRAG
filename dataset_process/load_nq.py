...
def get_qa_instruction(
    question: str, documents: List, use_cot, RETRIEVAL_TOKEN):
    if not question:
...
def get_instruction_dataset(dataset, idx, max_prompt_length, tokenizer, use_cot, RETRIEVAL_TOKEN, sample_answer=True):
    instruction_dataset = []
    for i in tqdm(idx):
...
def load_nq_dataset(input_path, max_prompt_length, tokenizer, retrieval_aware, use_cot, RETRIEVAL_TOKEN='<R>', dataset_seed=42):
    ...
    instruction_dataset_train = get_instruction_dataset(examples, train_index, max_prompt_length, tokenizer, use_cot, RETRIEVAL_TOKEN)
    instruction_dataset_test = get_instruction_dataset(examples, test_index, max_prompt_length, tokenizer, use_cot, RETRIEVAL_TOKEN)
